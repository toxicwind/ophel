...
- **Kimi 站点适配（首版）**：新增对 `www.kimi.com` 的完整站点接入，支持会话识别、会话列表读取、输入框注入与发送、回复抽取、大纲提取、导出链路、主题切换与生成态检测。
- **Kimi 会话历史全量同步**：支持从侧边栏“查看全部”进入历史会话页，滚动加载全部会话后同步到本地。

### ✨ 功能优化

- **Kimi 会话去重与标题提取优化**：兼容历史页分组重复展示（如置顶与按年分组），按会话 ID 去重并合并状态；补齐历史页标题选择器，提升会话名识别准确性。
- **Kimi 模型锁定配置接入**：站点设置中新增 Kimi 的模型锁定项，支持按关键词自动切换目标模型。

### 🐛 问题修复

- **Kimi 历史页同步后自动退出**：会话同步结束后自动点击历史页关闭按钮，回到主聊天视图，避免停留在历史页影响后续操作。
